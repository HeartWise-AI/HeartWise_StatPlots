...
class MetricsComputer:
    ...
    @classmethod
    @type_check(enabled=True, y_true=np.ndarray, y_pred=np.ndarray, dtypes={"y_true": np.int64, "y_pred": np.float64}, metric_func=Callable, n_iterations=int)
    def __bootstrap(cls, y_true: np.ndarray, y_pred: np.ndarray, metric_func: Callable, n_iterations: int = 1000) -> tuple[float, tuple[float, float]]:
        results = []
    ...
    @classmethod
    @type_check(enabled=True, y_true=np.ndarray, y_pred=np.ndarray, dtypes={"y_true": np.int64, "y_pred": np.float64})
    ...
    @classmethod
    @type_check(enabled=True, y_true=np.ndarray, y_pred=np.ndarray, dtypes={"y_true": np.int64, "y_pred": np.float64}, threshold=float)
    def confusion_matrix(cls, y_true: np.ndarray, y_pred: np.ndarray, threshold: float = 0.5) -> np.ndarray:
        cls.__check_ground_truth(y_true)
        cls.__check_prediction_values(y_pred)
    ...
    @classmethod
    @type_check(enabled=True, y_true=np.ndarray, y_pred=np.ndarray, dtypes={"y_true": np.int64, "y_pred": np.float64}, threshold=float)
    def sensitivity(cls, y_true: np.ndarray, y_pred: np.ndarray, threshold: float = 0.5) -> float:
        cls.__check_ground_truth(y_true)
        cls.__check_prediction_values(y_pred)
    ...
    @classmethod
    @type_check(enabled=True, y_true=np.ndarray, y_pred=np.ndarray, dtypes={"y_true": np.int64, "y_pred": np.float64}, threshold=float)
    def specificity(cls, y_true: np.ndarray, y_pred: np.ndarray, threshold: float = 0.5) -> float:
        cls.__check_ground_truth(y_true)
        cls.__check_prediction_values(y_pred)
        tn, fp, fn, tp = cls.confusion_matrix(y_true, y_pred, threshold).ravel()
    ...
    @classmethod
    @type_check(enabled=True, y_true=np.ndarray, y_pred=np.ndarray, dtypes={"y_true": np.int64, "y_pred": np.float64}, threshold=float)
    def ppv(cls, y_true: np.ndarray, y_pred: np.ndarray, threshold: float = 0.5) -> float:
        cls.__check_ground_truth(y_true)
        cls.__check_prediction_values(y_pred)
        tn, fp, fn, tp = cls.confusion_matrix(y_true, y_pred, threshold).ravel()
    ...
    @classmethod
    @type_check(enabled=True, y_true=np.ndarray, y_pred=np.ndarray, dtypes={"y_true": np.int64, "y_pred": np.float64}, threshold=float)
    def npv(cls, y_true: np.ndarray, y_pred: np.ndarray, threshold: float = 0.5) -> float:
        cls.__check_ground_truth(y_true)
        cls.__check_prediction_values(y_pred)
        tn, fp, fn, tp = cls.confusion_matrix(y_true, y_pred, threshold).ravel()
    ...
    def compute_classification_metrics(cls, y_true: np.ndarray, y_pred: np.ndarray, metrics: list, cutoff: str = 'default', bootstrap: bool = False, n_iterations: int = 1000) -> dict:
        ...
        for metric in metrics:
            if metric == ClassificationMetrics.ALL:
                metrics_to_compute.extend([m for m in ClassificationMetrics if m != ClassificationMetrics.ALL])
            else:
    ...
    @classmethod
    @type_check(enabled=True, y_true=np.ndarray, y_pred=np.ndarray, dtypes={"y_true": np.int64, "y_pred": np.float64}, metrics=list, bootstrap=bool, n_iterations=int)
    def compute_regression_metrics(cls, y_true: np.ndarray, y_pred: np.ndarray, metrics: list, bootstrap: bool = False, n_iterations: int = 1000) -> dict:
        results = {}
        metrics_to_compute = []

        for metric in metrics:
            if metric == RegressionMetrics.ALL:
                metrics_to_compute.extend([m for m in RegressionMetrics if m != RegressionMetrics.ALL])
            else:
                metrics_to_compute.append(metric)
        ...
        for metric in metrics_to_compute:
            ...
            if bootstrap and metric != RegressionMetrics.PEARSON_CORRELATION and metric != RegressionMetrics.SPEARMAN_CORRELATION:
                mean, ci = cls.__bootstrap(y_true, y_pred, func, n_iterations)
                results[metric.name.lower()] = {"mean": mean, "ci": ci}
            else:
                results[metric.name.lower()] = func(y_true, y_pred)
